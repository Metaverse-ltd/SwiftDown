...
public struct Theme {
    ...
    public var cursorColor: UniversalColor = UniversalColor.blue
    public var styles: [MarkdownNode.MarkdownType: Style] = [:]
    
    public init(_ name: String) {
        self.init()
        let bundle = Bundle(for: SwiftDown.self)
    ...
    // MARK: - Static methods
    public static func applyMarkdown(markdown: MarkdownNode, with theme: Theme) -> [NSAttributedString.Key:
                                                                                        Any] {
        guard let attributes = theme.styles[markdown.type]?.attributes else { return [:] }
        return attributes
    }
    
    public static func applyBody(with theme: Theme) -> [NSAttributedString.Key: Any] {
